...
class connection {
    ...
    /*
     * Multiple url excute
     * $nodes = array('url' => $url, 'cookie_send' => $cookie_send, 'session_send' => $session_send, 'follow' => $follow);
     */

    public function multiple_threads_get_content($nodes) {
        $mh = curl_multi_init();
        $curl_array = array();
        foreach ($nodes as $i => $node) {
            $url = $node['url'];
            (isset($node['session_send'])) ? $session_send = $node['$session_send'] : $session_send = NULL;
            (isset($node['post'])) ? $post = $node['post'] : $post = NULL;
            (isset($node['cookie_send'])) ? $cookie_send = $node['cookie_send'] : $cookie_send = NULL;
            (isset($node['follow'])) ? $follow = $node['follow'] : $follow = NULL;
            $cookies = $this->cookie;
            ...
            if ($session_send) {
                curl_setopt($curl_array[$i], CURLOPT_COOKIE, $session_send);
            }
            if ($post) {
            ...
            if (!$cookie_send) {
                curl_setopt($curl_array[$i], CURLOPT_COOKIEJAR, $cookies);
            } else {
                curl_setopt($curl_array[$i], CURLOPT_COOKIEFILE, $cookies);
            }
            if ($follow) {
